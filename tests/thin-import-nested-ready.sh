...
query_line=$(grep -nF 'exec prplmesh-20-0903 -- lxc query /1.0' \
...
proxy_line=$(grep -nF 'config device add' "$work/actions" | head -n 1 |
    cut -d: -f1)
import_line=$(grep -nF \
    'import '"$work"'/release/appliance.tar.zst prplmesh-20-0903' \
...
grep -F 'config device add prplmesh-20-0903 wmediumd-console proxy' \
    "$work/actions" | grep -F 'connect=tcp:10.99.0.42:8090' >/dev/null
grep -F 'config device add prplmesh-20-0903 controller-ui proxy' \
    "$work/actions" | grep -F 'connect=tcp:10.99.0.42:8091' >/dev/null
grep -Fq \
    'config device set prplmesh-20-0903 eth0 ipv4.address 10.99.0.42' \
    "$work/actions"
